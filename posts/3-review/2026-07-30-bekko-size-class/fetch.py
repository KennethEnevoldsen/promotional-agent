...
"""Evidence for the Bekko post. Run from the repo root:
    ...
    uv run posts/2026-08-06-bekko-size-class/fetch.py

Writes data.json (the full cohort) and the numbers into card.html's #card-data.

The size-class post: this card ranks the cohort and shows the large reference above
the ranking, hatched, so a reader can see where the field as a whole sits.
